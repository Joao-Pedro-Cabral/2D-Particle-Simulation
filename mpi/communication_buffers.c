...
void communication_buffers_init(communication_buffers_t *buffers, long ncside, int id, int p, long long npart) {
  ...
  particles_buffer_init(buffers->recv_particles_up, BLOCK_FRONTIER(id,p,ncside));
  particles_buffer_init(buffers->recv_particles_down, BLOCK_FRONTIER(id,p,ncside));
  particles_buffer_init(buffers->send_particles_up, BLOCK_FRONTIER(id,p,ncside));
  particles_buffer_init(buffers->send_particles_down, BLOCK_FRONTIER(id,p,ncside));
  buffers->particles_requests = malloc(sizeof(MPI_Request)*2*BLOCK_NUM_OF_NEIGHBORS(id,p,ncside));
}
