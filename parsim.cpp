  ...
  for(long i = 0; i < ncside + 2; i += ncside + 1) {
    for(long j = 0; j < ncside + 2; j++) {
      ...
      if(j == 0 || j == ncside + 1) {
        ...
        cells[ind].center.x = (j == 0) ? cells[ind2].center.x - side : cells[ind2].center.x + side;
        cells[ind].center.y = (i == 0) ? cells[ind2].center.y - side : cells[ind2].center.y + side;
  ...
  for (long iy = 0; iy < ncside; iy++) {
    ...
    for (long long j = 0; j < cells[i].par.size(); j++) {
      ...
      for (long long k = 0; k < cells[i].par.size(); k++) {
        ...
        resultant_x += force.x;
        resultant_y += force.y;
        if(cells[i].par[j].ind == 0) debug_accelerations(force, 0);
      }
      for (long long k = 0; k < 9; k++) {
        ...
        resultant_x += force.x;
        resultant_y += force.y;
        if(cells[i].par[j].ind == 0) debug_accelerations(force, ind);
      }
