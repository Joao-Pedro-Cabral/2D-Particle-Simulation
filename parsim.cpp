...
void fill_cells(double size, long ncside, long long n_part, const std::vector<particle_t>& par,
...
void compute_centers_of_mass(double size, long ncside, std::vector<cell_t>& cells) {
...
void compute_accelerations(long ncside, const std::vector<cell_t>& cells, std::vector<vec_t>& acc_vec) {
  long long l = 0;
  // TODO: WRAP!!!!!!!!!!!!!!!!!!!!!!!!!
  for (long i = 0; i < cells.size(); i++) {
    for (long long  j = 0; j < cells[i].par.size(); j++) {
...
void compute_new_positions_and_velocities(double size, long ncside, std::vector<cell_t>& cells, const std::vector<vec_t>& acc_vec) {
  long long l = 0;
  for (long i = 0; i < cells.size(); i++) {
    for (long long  j = 0; j < cells[i].par.size(); j++) {
...
void simulation(double side, long ncside, long long npart, long long nstep, const std::vector<particle_t>& par) {
